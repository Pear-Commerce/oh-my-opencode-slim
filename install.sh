...
if [ -n "$DESKTOP_APP" ]; then
  ...
  echo "  3. Confirm the OpenCodex proxy is running at http://127.0.0.1:10100/."
  echo "     OpenCode uses the local opencodex provider; upstream credentials stay"
  echo "     in OpenCodex, not in OpenCode."
  echo "  4. Confirm the GLM 5.2 model is visible in the Desktop model picker:"
  echo "       opencodex/glm-5p2"
  echo "     If not, refresh models in the picker."
else
  echo "  1. Confirm the OpenCodex proxy is running at http://127.0.0.1:10100/."
  echo "     Upstream OpenAI/Fireworks credentials are managed by OpenCodex."
  echo "  2. Refresh the model list and confirm GLM 5.2 is visible:"
  echo "       opencode models --refresh"
  echo "     You should see 'opencodex/glm-5p2'."
