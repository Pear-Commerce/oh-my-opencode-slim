...
3. **Convergence gates (loop termination).** Replaces the model yes/no
   ...
   - **`adjudicator` gate** — spawns a cheap LLM with a prompt (and optional
     file attachments) that must respond `PASS` or `FAIL` on its first line.
     Uses the configured Oracle model by default. Files can be attached for
     document review.
